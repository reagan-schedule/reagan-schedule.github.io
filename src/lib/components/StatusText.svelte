...
<span class={className}>
    {#if then.when}
        {@const duration = now.until(then.when, { roundingMode: 'ceil', largestUnit: 'hour', smallestUnit: 'minute' })}
        {@const parts = [duration.hours && hour.format(duration.hours), minute.format(duration.minutes)].filter((x) => x !== 0)}
